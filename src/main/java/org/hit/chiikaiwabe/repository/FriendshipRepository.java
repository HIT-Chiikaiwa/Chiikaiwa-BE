...
public interface FriendshipRepository extends JpaRepository<Friendship, String> {
    ...
    @Query("SELECT f FROM Friendship f " +
            ...
            "WHERE (f.requester.id = :userId OR f.receiver.id = :userId) " +
            "AND f.status = 'ACCEPTED' " +
            "AND (LOWER(CASE WHEN f.requester.id = :userId THEN f.receiver.firstName ELSE f.requester.firstName END) LIKE :keyword " +
            "OR LOWER(CASE WHEN f.requester.id = :userId THEN f.receiver.lastName ELSE f.requester.lastName END) LIKE :keyword)")
